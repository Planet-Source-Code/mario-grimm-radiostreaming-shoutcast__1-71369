...
Global WinAmpIsInstalled As Boolean
Global sWinAmpLocation As String
Type FILETIME
...
Public Function IsWinampLocated() As Boolean
Dim sString As String
sString = GetStringValue("HKEY_CLASSES_ROOT\Applications\Winamp.exe\shell\open\command", "")
If sString <> "" Then
    sString = Left$(sString, InStr(1, sString, Chr$(0)) - 1)
    sString = Replace$(sString, Chr$(34), "")
    sString = Replace$(sString, "%1", "")
    If Dir$(sString, vbNormal) <> "" Then
        IsWinampLocated = True
    End If
    sWinAmpLocation = sString
End If
End Function
